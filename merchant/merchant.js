...
(function () {
  ...
  function viewInventory() {
    ...
    const headers = food ? ["Item", "Price (â¹)", "Qty available", ""] : ["Item", "Price (â¹)", ""];
    ...
    const rows = products.map((p) => {
      ...
      if (food) cells.push(el("td", {}, p.qty !== undefined ? String(p.qty) : el("span", { class: "muted" }, "â")));
  ...
  function editProduct(p) {
    ...
    const nameEl  = el("input", { value: p ? p.name  : "", placeholder: "e.g. Masala Dosa, Paracetamol stripâ¦" });
    const priceEl = el("input", { type: "number", value: p ? p.price : "", placeholder: "0", min: "0" });
    const qtyEl   = el("input", { type: "number", value: p && p.qty !== undefined ? p.qty : "", placeholder: "e.g. 20", min: "0" });

    const fields = [
      el("div", { class: "field" }, [el("label", {}, "Item name"), nameEl]),
      el("div", { class: "field" }, [el("label", {}, "Price (â¹)"), priceEl]),
    ];
    if (food) {
      ...
      footer: [
        ...
        el("button", { class: "btn primary", onClick: async () => {
          ...
          try {
            ...
            if (food && qtyEl.value !== "") payload.qty = Number(qtyEl.value);
    ...
    const body = [
      ...
      el("div", { style: "max-width:400px;margin:0 auto" }, [
        ...
        el("div", { style: "background:#fff9f5;border:1px solid #ffe0c8;border-radius:12px;padding:20px;margin-bottom:16px" }, [
          ...
          ...[
            ["1ï¸", "Customer scans QR code with their phone"],
            ["2û", "First-time? They install the app â your store loads automatically"],
            ["3û", "Returning? Your store is added to their existing app"],
            ["4û", "They can collect stores from multiple merchants over time"],
          ].map(([ico, text]) =>
